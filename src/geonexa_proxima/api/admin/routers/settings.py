...
async def env_diff(admin: Admin, db: Engine) -> list[dict[str, Any]]:
    ...
    rows = await fetch_all(
        db,
        text(
            "SELECT key, value, env_default, scope, is_secret, updated_by, updated_at"
            " FROM app_settings"
            " WHERE updated_by IS NOT NULL AND updated_by <> 'seed'"
            " AND value IS DISTINCT FROM env_default ORDER BY updated_at DESC"
        ),
    )
    # Маскируем ровно так же, как в общем списке. Раньше эта ручка отдавала
    # значения как есть: сегодня секретов в таблице нет, но колонка
    # `is_secret` существует, и первая же секретная настройка утекла бы
    # отсюда — в браузер и в логи node-слоя.
    for row in rows:
