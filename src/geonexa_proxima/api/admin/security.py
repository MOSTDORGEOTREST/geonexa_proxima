...
def client_ip(request: Request) -> str:
    """IP клиента. Заголовку прокси верим, только если так велели настройки.

    `X-Forwarded-For` пишет кто угодно. Пока ему верили безусловно, ограничение
    попыток входа обходилось одной строкой: новый выдуманный адрес на каждый
    запрос — и перебор пароля идёт со скоростью сети, а в аудите остаются
    придуманные адреса, по которым его не видно и постфактум.
    """
