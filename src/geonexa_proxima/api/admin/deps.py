...
def container_of(request: Request) -> Any:
    """Контейнер приложения — или 503, если он не собрался.

    Через него админка достаёт бота и сервис профилей. Оба нужны там, где
    администратор действует за подписчика: подтверждает доступ и правит
    профиль интересов группы.
    """

    container = getattr(request.app.state, "container", None)
    if container is None:
        raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, detail="Сервис не сконфигурирован")
    return container


AppContainer = Annotated[Any, Depends(container_of)]


def profile_service(request: Request) -> Any:
    """Сервис профилей: правка описания обязана перекомпилировать `compiled_text`.

    Прямой UPDATE по `subscriber_profiles` этого не делает — и профиль,
    отредактированный в админке, продолжал бы ранжировать материалы по
    прошлому описанию. Разойтись эти два поля могут молча: в интерфейсе видно
    новое описание, а в выдаче работает старое.
    """
    ...
    try:
        ...
        raise HTTPException(
            status.HTTP_503_SERVICE_UNAVAILABLE, detail=f"Профили недоступны: {error}"
        ) from error


Profiles = Annotated[Any, Depends(profile_service)]


def telegram_bot(request: Request) -> Any:
    container = container_of(request)
    ...
    return container.telegram_bot()


Bot = Annotated[Any, Depends(telegram_bot)]
