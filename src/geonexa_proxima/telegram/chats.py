...
Чат заводится заявкой, а не подписчиком: статус `pending`, дайджесты
выключены, диспетчер его не видит. Работать он начинает после того, как
администратор подтвердит его в админке и заполнит профиль интересов —
профиль группы ведёт администратор сервиса, а не участники чата.
"""
...
from geonexa_proxima.domain import PRESENT_BOT_STATUSES, UserStatus
from geonexa_proxima.services.container import Container
from geonexa_proxima.telegram.onboarding import CHAT_PENDING
...
def register_chat_router(container: Container) -> Router:
    ...
    async def on_membership_changed(event: ChatMemberUpdated) -> None:
        ...
        if known is None:
            if not present or not settings.telegram_auto_register_chats:
                # Автозаведение выключено — фиксируем факт, но не создаём даже
                # заявку: иначе админ обнаружит в очереди чаты, которых не ждал.
                log.info(
                ...
                return
            user, created = await repository.register_chat(
                identity,
                bot_status=status,
            ...
            if created and user.status is UserStatus.PENDING:
                await _announce_pending(event, chat.id)
            return
    ...
    async def _announce_pending(event: ChatMemberUpdated, chat_id: int) -> None:
        """Сказать в чате, что бот здесь и ждёт подтверждения.

        Молчаливый бот в группе выглядит как сломанный: его добавили, он не
        отвечает, и через день его выкидывают. Отправка не критична — в канале
        без права постить она законно не проходит, и это не повод ронять
        обработку апдейта.
        """

        try:
            await event.bot.send_message(chat_id, CHAT_PENDING)
        except Exception as error:
            log.info("В чат %s не удалось написать о заявке: %s", chat_id, error)
