...
GUIDE: tuple[GuideSection, ...] = (
    ...
    GuideSection(
        title="Явные темы: указывайте оба языка через «;»",
        body=(
            "Тема из списка тем работает двумя способами. По смыслу — как и "
            "описание, язык здесь не важен. И буквально: точное вхождение в "
            "название или аннотацию даёт материалу отдельную прибавку.",
            "Корпус собирается с arXiv, OpenAlex и GitHub и почти весь "
            "английский, поэтому русский термин буквально не совпадёт никогда. "
            "Пишите оба написания через точку с запятой — сверка проверит "
            "каждое, а по смыслу тема возьмётся целиком.",
        ),
        good=(
        ...
        bad=(
            (
                "разжижение грунтов",
                "по смыслу сработает, но буквальной прибавки не даст: в "
                "английских аннотациях этих слов нет",
            ),
        ),
    ),
    GuideSection(
        ...
        body=(
            ...
            "сверяется буквально, поэтому английское написание ей нужно "
            "обязательно — иначе она не сработает вовсе.",
        ),
...
def _interest_notes(interests_block: str) -> list[Note]:
    """Замечания к явным темам: язык написания решает, сработают ли они."""
    ...
    for interest in parse_interests(interests_block):
        ...
        notes.append(
            Note(
                "error" if negative else "warning",
                (
                    f"«{term}»: минус-тема без английского написания не сработает "
                    "вовсе — буквальная сверка идёт по английскому тексту статей. "
                    "Допишите вариант через «;»."
                )
                if negative
                else (
                    f"«{term}»: буквальная прибавка не сработает — в английских "
                    "аннотациях этих слов нет. Допишите английский вариант через "
                    "«;», например «liquefaction; разжижение грунтов»."
                ),
                subject=term,
            )
        )
