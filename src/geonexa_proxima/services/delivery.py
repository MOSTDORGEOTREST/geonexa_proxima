...
class DeliveryQueue:
    ...
    async def mark_sending(self, job_id: UUID) -> bool:
        """Взять задание в работу. ``False`` — задание уже не наше.

        Условие по `claimed_by` — это фенсинг. `release_stale` возвращает в
        очередь всё, что висит дольше получаса, и если пачка отправлялась
        дольше (лимиты Bot API растягивают полсотни постов на минуты), её
        хвост успевает уехать другому воркеру. Без проверки владельца оба
        отправляли бы одни и те же сообщения, и подписчик получал бы дайджест
        дважды.
        """
        ...
        result = await self._execute(
            ...
            "WHERE id=:id AND claimed_by=:worker AND status IN ('claimed','sending')",
            {"id": str(job_id), "worker": self.worker_id},
        )
    ...
    async def mark_failed(
        ...
        *,
        ...
        # «Бота заблокировали» и «чат не найден» ретраить бессмысленно: ни одна
        # из пяти попыток ничего не изменит, а очередь всё это время держит
        # задание и тратит вызовы Bot API.
        exhausted = "attempts >= max_attempts" if not permanent else "true"
        async with self.engine.begin() as connection:
            row = (
                await connection.execute(
                    text(
                        ...
                        UPDATE delivery_jobs
                           SET status = CASE WHEN {exhausted}
                                             THEN 'failed' ELSE 'queued' END,
                               last_error = :error,
                               next_retry_at = CASE WHEN {exhausted} THEN NULL
                               ...
                               finished_at = CASE WHEN {exhausted}
    ...
    async def sent_positions(self, job_id: UUID) -> set[int]:
        """Какие блоки этого задания уже ушли в чат.

        Ретрай начинал задание сначала: карточки 1-5 уходили, шестая падала по
        429, и через минуту подписчик получал те же пять карточек ещё раз. Всё
        нужное для честного продолжения уже писалось в журнал сообщений —
        оставалось его прочитать.
        """

        async with self.engine.connect() as connection:
            rows = await connection.execute(
                text(
                    "SELECT position FROM delivery_messages "
                    "WHERE delivery_job_id = :id AND status = 'sent'"
                ),
                {"id": str(job_id)},
            )
