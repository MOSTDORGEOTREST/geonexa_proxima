...
class LocalQwen3Embedder:
    ...
    async def embed_queries(self, texts: Sequence[str]) -> list[list[float]]:
        """Пачка запросов за один прогон модели.

        Грани профиля эмбеддятся вместе на каждый профиль в каждом прогоне
        диспетчера: пять отдельных вызовов означали бы пять проходов по модели
        вместо одного батча.
        """

        if not texts:
            return []
        return await asyncio.to_thread(self._encode, list(texts), True)
...
class OpenAICompatibleEmbedder(AsyncHTTPProvider):
    ...
    async def embed_queries(self, texts: Sequence[str]) -> list[list[float]]:
        if not texts:
            return []
        return await self._embed([format_query(text, self.query_instruction) for text in texts])
