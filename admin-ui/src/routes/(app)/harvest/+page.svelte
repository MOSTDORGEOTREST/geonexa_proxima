...
<!-- Прогон запускают отсюда: сюда приходят, когда сбор «ничего не находит»,
     и заставлять ради одной кнопки идти в «Расписания» — лишний шаг. -->
<section class="run">
	<div>
	...
	<div class="run-actions">
		...
		<form method="POST" action="?/collect" use:enhance>
			...
			<input
				type="hidden"
				name="parameters"
				value={JSON.stringify({ lookback_hours: 720, limit_per_source: 200 })}
			/>
	...
	{#if data.activeRun}
		...
		<div class="flash active">
			...
			<form method="POST" action="?/abort" use:enhance>
