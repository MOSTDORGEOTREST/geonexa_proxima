...
def test_onboarding_texts_are_russian() -> None:
    """Приветствия живут константами и мимо `_outgoing_strings` не ловятся.

    Отправляют их два процесса — бот и админ-API, — поэтому текст вынесен в
    отдельный модуль, и проверять его надо там же, а не по месту вызова.
    """
    ...
    latin_only = sorted(
        ...
        if LATIN_WORD.search(value) and not CYRILLIC.search(value)
    )
    assert not latin_only, latin_only
