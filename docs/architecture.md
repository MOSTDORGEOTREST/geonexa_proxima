...
facets  = [весь профиль, тема 1, тема 2, …]  # разбор compiled_text
vectors = profile_vectors.get(profile, version, facet) or rebuild()
hits    = [search(v, limit) for v in vectors]        # запрос на каждую грань
window: created_at >= since                          # и для SQL, и для векторов
semantic = max(близость по всем граням)              # максимум, не среднее
gate: global_score >= threshold ИЛИ близость >= FACET_OVERRIDE (косинус)
cap: топ-N по формуле без реранкера                  # RERANK_LIMIT
reranked = reranker.score(грань, документы этой грани)  # узкий запрос
personal = 0.40·semantic + 0.25·reranker + 0.25·global + 0.10·interests
filter(personal >= profile.min_personal_score)       # до квоты, а не после
quota(PERSONAL_FACET_MIN_SLOTS) → top N     # каждой грани — своё место
explain(light LLM, весь профиль)            # роль explainer
digest = digests.create(status='ready')
delivery_jobs.enqueue(digest, channel=personal|group)   ← флоу здесь заканчивается
```

Почему граней несколько. Профиль из нескольких областей в одном векторе — это
центроид между ними: статья, глубоко попадающая в одну тему, получает средний
косинус, потому что центроид оттянут остальными темами. Поэтому ищем ещё и
каждой темой отдельно, а близость берём максимумом. Реранкер спрашивается той
гранью, которой материал нашёлся, — для него запрос из четырёх несвязанных тем
такой же шум, как и для эмбеддера. Объяснение при этом строится по всему
профилю: узкой гранью можно найти материал, но нельзя объяснить, зачем он
человеку целиком. Подробности и настройки — в README, раздел «Поиск по граням
профиля».

Два ограничения, без которых схема разъезжается. Окно `since` применяется и к
векторным попаданиям: списка «уже отправленного» в системе нет, и без него
старый материал с высоким косинусом возвращался бы в каждый дайджест. И потолок
`PERSONALIZATION_RERANK_LIMIT` перед кросс-энкодером: граней бывает до тридцати
двух, каждая тянет свою выборку, а реранкер считает пару на каждый материал.
...
`pending`, и он появляется в админке на модерацию. В сам чат уходит короткое
сообщение о том, что бот ждёт подтверждения: молчащий бот в группе выглядит
сломанным.

Возврат бота в чат, который однажды подтвердили, доступ не переоткрывает и не
требует второго подтверждения: отметка живёт в `subscribers.meta.approved`
отдельно от статуса. Иначе повторное добавление снимало бы блокировку,
поставленную администратором, — а переезд группы в супергруппу каждый раз
создавал бы новую заявку.
