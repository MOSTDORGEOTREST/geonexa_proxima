...
> живой базы: вход и токены, дашборд, подписчики и профили, чаты, тарифы и
...
POST   /api/admin/subscribers/{id}/approve       -- pending -> active
...
PATCH  /api/admin/profiles/{id}                  -- description, digest_settings, пороги, формат, расписание
...
POST   /api/admin/profiles/{id}/interests        {query|topic_id, polarity, weight}
...
| **Профиль подписчика** | Описание, интересы ±вес, выученные сигналы, формат доставки, расписание, пороги, «Пересобрать», «Превью дайджеста», «Отправить сейчас» |
...
| **Расписания** | Все расписания одной таблицей, редактор cron с человеческим описанием и следующими пятью запусками, Run now, ссылка в Prefect UI, история `flow_runs` |
